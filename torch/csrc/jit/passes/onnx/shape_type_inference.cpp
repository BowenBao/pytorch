...
std::shared_ptr<Graph> ONNXSetDynamicInputShape(
    std::shared_ptr<Graph>& graph,
  ...
  std::unordered_map<std::string, ::c10::ShapeSymbol> name_to_sym;
...
std::shared_ptr<Graph> ONNXAssignOutputShape(
    std::shared_ptr<Graph>& graph,
...
std::shared_ptr<Graph> ONNXShapeTypeInference(
    std::shared_ptr<Graph>& graph,
    int opset_version) {
  auto retval = graph->copy();
